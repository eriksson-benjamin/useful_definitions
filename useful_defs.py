...
def multipage(filename, figs=None, dpi=200, check=True, tight_layout=True):
    ...
    if check:
        if os.path.exists(filename): 
            ...
            if inp != 'y': return 0
            else: print('Overwriting file.')
    pp = PdfPages(filename)
    if figs is None:
        figs = [plt.figure(n) for n in plt.get_fignums()]
    for fig in figs:
        if tight_layout: fig.savefig(pp, format = 'pdf', bbox_inches = 'tight', pad_inches = 0)
...
def multifig(filename, check=True, tight_layout=True, ext='pdf'):
    ...
    for i, fig in enumerate(figs):
        ...
        if tight_layout: fig.savefig(f'{filename}_{i}.{ext}', format=ext, bbox_inches='tight', pad_inches=0)
        else: fig.savefig(f'{filename}_{i}.{ext}', format=ext)
...
def hist_1D_s(x_data, title = '', label = 'data set', log = True, bins = 0, 
    ...
    hist = plt.hist(x_data, label = label, bins = bins, log = log, histtype = histtype, density = normed)
    plt.title(title)
...
def hist_2D(x_data, y_data, title = '2D histogram', log = True, 
    ...
    if log == True:
        plt.hist2d(x_data, y_data, bins, range = [x_range, y_range], norm = LogNorm(), cmap = my_cmap, vmin = 1)
    else:
        plt.hist2d(x_data, y_data, bins, range = [x_range, y_range], cmap = my_cmap, vmin = 1)
...
def pickler(file_name, to_pickle, check = True):
    ...
    if inp == 'y': print('File was overwritten.') 
...
def make_gaussian(area, mu, sigma, bins, upscale = np.array([0, 0, 0])):
    ...
    if (upscale == np.array([0, 0, 0])).all(): 
        ...
        gaus = np.diff(bins)[0]/np.diff(bins_new)[0] * area * np.diff(s_erf) / 2
...
def return_shots(n_of_shots = 0, n_max = np.inf, n_min = 0, shot_min = 0, 
    ...
    if shot_max == 0: shot_max = ppf.pdmsht()
    if n_max == 0 and n_min == 0: fission_chambers = False 
    else: fission_chambers = True
    
    delta_shots = shot_max - shot_min
    for i, shot_number in enumerate(range(shot_min, shot_max+1)):
        if slow: time.sleep(1)
        # Print progress
        if n_of_shots == 0: print(f'Progress: {i+1}/{delta_shots} JPN#{shot_number}')
        else: print(f'Progress: {i}/{n_of_shots} JPN#{shot_number}')
        # Check length of the list
        if n_of_shots != 0 and len(list_of_shots) == n_of_shots: break
        ...
        if fission_chambers:
            ...
            if n_neutrons < n_min or n_neutrons > n_max: continue
        
        # Check if TOFu has data on all channels
        if TOFu_complete:
            for s1 in dfs.get_dictionaries('S1').keys():
                if np.shape(dfs.get_times(detector_name = s1, shot_number = shot_number)) == (): continue
...
def import_pulses(board = '', channel = '', path = '', record_length = 64, full_path = 0):
    ...
    '''
    # Import the pulses
    if full_path == 0: file_name = path + 'M11D-B' + board + '_DT' + channel
...
def import_times(board = '', channel = '', path = '', full_path = 0):
    ...
    '''
    if full_path == 0: fname = path + 'M11D-B' + board + '_TM' + channel
    else: fname = full_path
    if board in ('06', '07', '08', '09', '10'):
        T = np.fromfile(fname, dtype = 'uint64') * 0.5 # 0.5 = conversion factor to ns for ADQ412's
        return T
...
def import_offset(board = '', path = '', full_path = 0):
    ...
    '''
    if full_path == 0: fname = path + 'M11D-B' + board + '_OFF'
    else: fname = full_path
...
def make_phs(shot_number, detector_name, return_data = False, ly_function = 'gatu'):
    ...
    pulses = dfs.get_pulses(board = board, channel = channel, shot_number = shot_number)
    ...
    plt.annotate(f'Counts: {np.sum(hist_vals)}', xy=(0.7, 0.95), xycoords='axes fraction')
    ...
    plt.title(f'TOFu {detector_name}, JPN {shot_number}')
    plt.annotate(f'Counts: {np.sum(ly_vals)}', xy=(0.7, 0.95), xycoords='axes fraction')
    ...
    plt.annotate(f'Counts: {np.sum(pr_vals)}', xy = (0.7, 0.95), xycoords = 'axes fraction')
    plt.title(f'TOFu {detector_name}, JPN {shot_number}')
...
def grab_from_excel(file_name, rows = [0, 0], columns = ['A', 'A'], sheet_name = None):
    ...
    cols = [col2num(columns[0]) - 1, col2num(columns[1])] # Translate letter -> column number
    ...
    if workbook.nsheets > 1: sheet = workbook.sheet_by_name(sheet_name)
    else: sheet = workbook.sheet_by_index(0)

    # Initialize list    
    new_list = np.zeros([select_rows[-1] - select_rows[0] + 1, cols[-1] - cols[0]])
    
    # Read values from spread sheet and save to new_list
    for i, row in enumerate(select_rows): 
        row_vals = sheet.row_slice(rowx = row, start_colx = cols[0], end_colx = cols[1])
...
def good_shots(shots):
    ...
    good_shots = np.loadtxt('/home/beriksso/TOFu/analysis/benjamin/other/return_shot_numbers/all_shots.txt')
...
def plot_nes_pickle(file_name, show_background=True, title=None, fig_name='tof spectrum', log2d=False):
    ...
    plt.plot(bins, tof, color=color, marker='.', markersize=1.5, linestyle='None')
...
def random_colors(size=1, seed=0):
    ...
    if size == 1:
        ...
        colors = [[np.random.uniform(0, 1), np.random.uniform(0, 1), np.random.uniform(0, 1)] for i in range(size)]
...
def codas_gas_code(code):
    ...
    elif code == 31: return 'H2'
    elif code == 32: return 'D2'
    elif code == 33: return 'Tritium'
    elif code == 34: return 'He3'
    elif code == 35: return 'He4'
    elif code == 36: return 'CD4'
    elif code == 37: return 'CH4'
    elif code == 38: return 'O2'
    elif code == 39: return 'Ar'
    elif code == 45: return 'Xe'
    elif code == 22: return 'Kr'
    elif code == 23: return 'N2'
    elif code == 24: return 'SiH4'
    elif code == 25: return 'Ne'
    elif code == 27: return 'C2H4'
    elif code == 28: return 'C2H6'
    elif code == 29: return 'C13H4'
    elif code == 40: return 'C3H8'
    elif code == 41: return 'TMB'
    else:
        print('Unknown code.')
        return 0
...
def plot_discharge(shot_number, t=[None, None]):
    ...
    for axis in ax.flatten(): axis.tick_params(which='major', length=4, width=1)
    
    colors = ['k', 'C0', 'C1', 'C2', 'C3']
    ls = ['-', '--', '-.', 'dotted', (0, (3, 5, 1, 5, 1, 5))]
    lw = [1, 2, 2, 2, 2]
    if t[0]!=None and t[1]!=None: ax[0, 0].set_xlim(t[0], t[1])
    
    '''
    Gas puffing
    '''
    gas_codes = ppf.ppfget(dda='GASH', dtyp='CGAS', pulse=shot_number)[2].astype('int')
    for i, gas_code in enumerate(gas_codes):
        ...
        ax[0, 0].plot(gas[4], gas[2], color=colors[i], linestyle=ls[i], label=gas_type, linewidth=lw[i])
    ax[0, 0].legend()
    ...
    if len(nbi[2] > 0):
        ...
        # Plot NBI
        ax[1, 0].plot(nbi[4], nbi[2]/1E6, color=colors[0], linestyle=ls[0], label=f'{gas_type[0]} NBI')

    if len(icrh[2] > 0):
        ax[1, 0].plot(icrh[4], icrh[2]/1E6, color=colors[1], linestyle=ls[1], label='ICRH', linewidth=lw[1])
    if len(lhcd[2] > 0):
        ax[1, 0].plot(lhcd[4], lhcd[2]/1E6, color=colors[2], linestyle=ls[2], label='LHCD', linewidth=lw[2])
    ax[1, 0].legend()
    ...
    if len(hrts_ne[2]) > 0:
        ax[1, 1].plot(hrts_ne[4], hrts_ne[2], color=colors[0], linestyle=ls[0], label='HRTS')
    if len(lidx_ne[2]) > 0:
        ax[1, 1].plot(lidx_ne[4], lidx_ne[2], color=colors[1], linestyle=ls[1], label='Lidar')
    ...
    if len(hrts_te[2] > 0):
        ax[2, 1].plot(hrts_te[4], hrts_te[2]/1E3, color=colors[0], linestyle=ls[0], label='HRTX')
    if len(kk3_te[2] > 0):
        ax[2, 1].plot(kk3_te[4], kk3_te[2]/1E3, color=colors[1], linestyle=ls[1], label='KK3')
    if len(lidar_te[2] > 0):
        ax[2, 1].plot(lidar_te[4], lidar_te[2]/1E3, color=colors[2], linestyle=ls[2], label='LIDX')
    if len(lid2_te[2] > 0):
        ax[2, 1].plot(lid2_te[4], lid2_te[2]/1E3, color=colors[3], linestyle=ls[3], label='LID2')
    if len(ecm1_te[2] > 0):
        ax[2, 1].plot(ecm1_te[4], ecm1_te[2]/1E3, color=colors[4], linestyle=ls[4], label='ECM1')
...
def ohmic_heating(shot_number, t0=40, t1=120):
    ...
    ne_t = ne[4]
    n_e = ne[2]
    if len(n_e)==0: raise Exception(f'Electron density (HRTX/NE0) data not available for JPN {shot_number}.')
    
    # Get electron temperature [eV]
    ecm1_te = ppf.ppfget(dda='ECM1', dtyp='TCOM', pulse=shot_number)
    Te_t = ecm1_te[4]
    T_e = ecm1_te[2]
    if len(T_e)==0: raise Exception(f'Electron temperature (ECM1/TCOM) not available for JPN {shot_number}.')
    
    # Get plasma current [A]
    I = ppf.ppfget(dda='MAGN', dtyp='IPLA', pulse=shot_number)
    I_p = -I[2]
    Ip_t = I[4]
    if len(Ip_t)==0: raise Exception(f'Plasma current (MAGN/IPLA) not available for JPN {shot_number}.')
    ...
    lambda_d = np.sqrt(constants.epsilon_0*constants.k*T_e/(n_e*constants.e**2))
    
    # Calculate coulomb logarithm
    ln_L = np.log(12*np.pi*n_e*lambda_d**3)
    
    # Calculate resisitivity [Ohm m]
    eta = np.pi*constants.e**2*np.sqrt(constants.electron_mass)/((4*np.pi*constants.epsilon_0)**2*(constants.k*T_e)**(3/2))*ln_L 
